...
//    , ocfDeviceType: "x.com.st.d.remotecontroller"
		...
		capability "Refresh"
		capability "Sensor"
        capability "futuredance12594.poolSpaMode8"
		capability "futuredance12594.waterTemperature1"
        capability "futuredance12594.poolSetpoint2"
        capability "futuredance12594.spaSetpoint2"
		capability "futuredance12594.heaterState1"
        capability "futuredance12594.switchCapOne1"
        capability "futuredance12594.switchCapTwo1"
        capability "futuredance12594.switchCapThree1"
        capability "futuredance12594.switchCapFour1"
        capability "futuredance12594.switchCapFive1"
        capability "futuredance12594.variableSpeedPoolPump2"
		capability "futuredance12594.airTemperature1"
		capability "futuredance12594.solarTemperature1"
...
def getLOCAL_ATTR_NAME (int id) {
	def swNames = ["switchOne","switchTwo","switchThree","switchFour","switchFive","poolSpaMode","swVSP1","swVSP2","swVSP3","swVSP4","waterTemperature","WaterTemperature", "poolSetpoint",    "spaSetpoint",     null,             null,             "heaterState",              "heaterState"]
	return swNames[id - 1]
}
...
private process84Event(byte [] payload) {
	...
	if (POOL_SPA_COMBO) {
        if ( EXPANSION_5043 ) {
    		PSMode = ((payload[POOL_SPA_MODE_84] & 0x01) == 0) ? "on" : "off"
        } else {
    		PSMode = ((payload[SWITCHES_84] & 0x08)) ? "on" : "off"    // For P4xxx expansion report Sw4
        }
		rslt.addAll(createMultipleEvents(POOL_SPA_EP, PSMode, PSMode))
	}
	...
	rslt.addAll(getWaterTempEvents(val, PSMode))
	...
	rslt << createEvent(name: "airTemperature", value: temp, unit: "F", displayed: true, isStateChange: true)
	...
	rslt << createEvent(name: "solarTemperature", value: temp, unit: "F", displayed: true, isStateChange: true)
...
private List getWaterTempEvents(temp, PSMode) {
	log("DEBUG", DBG_MANF, "----- getWaterTempEvents temp=$temp  PSMode=$PSMode")
	def cmds = []
...
def zwaveEvent(physicalgraph.zwave.commands.basicv1.BasicGet cmd) {
	...
	def cmds = []
	int val = device.currentValue("switchOne").equals("on") ? 0xFF : 0
...
private initUILabels() {
	...
	sendEvent(name: "vspSpeed", value: 2, displayed: false, descriptionText:"Initial Value")
	sendEvent(name: "waterTemperature", value: 50, displayed: false, descriptionText:"Initial Value")
	sendEvent(name: "airTemperature", value: 60, displayed: false, descriptionText:"Initial Value")
	sendEvent(name: "solarTemperature", value: 70, displayed: false, descriptionText:"Initial Value")
	sendEvent(name: "switchOne", value: "on", displayed: false, descriptionText:"Initial Value")
	sendEvent(name: "switchTwo", value: "on", displayed: false, descriptionText:"Initial Value")
	sendEvent(name: "switchThree", value: "on", displayed: false, descriptionText:"Initial Value")
	sendEvent(name: "switchFour", value: "on", displayed: false, descriptionText:"Initial Value")
	sendEvent(name: "switchFive", value: "on", displayed: false, descriptionText:"Initial Value")
	sendEvent(name: "spaSetpoint", value: 55, displayed: false, descriptionText:"Initial Value")
	sendEvent(name: "poolSetpoint", value: 66, displayed: false, descriptionText:"Initial Value")
}
...
//    log("DEBUG", "----- updated() .. devStr=$devStr")
	[]
}
...
//	def CHILD_SWITCH_NAMESPACE = "futuredance12594"
...
private Object addOrReuseChildDevice(childNo, name, nameSpace, deviceName, List oldChildren, parentComp){
	def Object devObj = null
	def compName = "ep${childNo}"
	if ( childNo > 100 ) compName = "Circuit ${childNo-100}"
	def dni = "${device.deviceNetworkId}-${compName}"
	log("DEBUG", DBG_CHILD, "addOrReuseChildDevice dni=${dni} oldChildren.size=${oldChildren.size} oldChildren: ${oldChildren}  parentComp: ${parentComp}")
...
//			log("DEBUG", 
			log.trace(
            "----- addChildDevice(namespace=\"${nameSpace}\",DTH Name=\"${deviceName}\", dni=\"${dni}\", hubId=null,"+
				  "properties=[completedSetup: true, label: \"${name}\","+
				  "isComponent: ${parentComp}"+
//                  ", componentName: \"${compName}\", componentLabel: \"${name}\""+
                  "]")
			// SmartThings
...
private List setPoolSpaModeInternal(val) {
	log("DEBUG", "+++++ setPoolSpaMode( $val )")
	def cmds = []
	cmds.addAll(setChanState(POOL_SPA_CHAN, val.equals("on") ? 0xFF : 0 ))
	cmds
}
...
private List setSpaModeInternal() {
	...
	def cmds = []
	cmds.addAll(setChanState(POOL_SPA_CHAN, 0xFF))
	cmds
}
...
private List setPoolModeInternal() {
	...
	def cmds = []
	cmds.addAll(setChanState(POOL_SPA_CHAN, 0))
	cmds
}
...
//		Eval.me("new physicalgraph.device.HubAction(${str}"),
...
def List setSwitchX(sw, val)  {
	log("DEBUG", "+++++ setSwitchX( $sw, $val)")
	prepareCommandsAndLog(setChanState(sw, val=="on" ? 0xFF : 0), true)
}
def List setSwitchOne(val)    { prepareCommandsAndLog(setSwitchX(1, val), true) }
def List setSwitchTwo(val)    { prepareCommandsAndLog(setSwitchX(2, val), true) }
def List setSwitchThree(val)  { prepareCommandsAndLog(setSwitchX(3, val), true) }
def List setSwitchFour(val)   { prepareCommandsAndLog(setSwitchX(4, val), true) }
def List setSwitchFive(val)   { prepareCommandsAndLog(setSwitchX(5, val), true) }

def List setPoolSpaMode(val)  { prepareCommandsAndLog(setPoolSpaModeInternal(val), true) }
...
def List setSpaSetpoint(degrees)  { prepareCommandsAndLog(setSpaSetpointInternal("${degrees}".toDouble()), true) }
def List setPoolSetpoint(degrees) { prepareCommandsAndLog(setPoolSetpointInternal("${degrees}".toDouble()), true) }
...
//	log("DEBUG", DBG_CMDS, "+++++ formatAndLog parm[${parm.size}] dly=$dly responseFlg=${responseFlg}")
	...
	lst.eachWithIndex {l, index ->
		...
		} else if (l instanceof physicalgraph.device.HubAction) {
...
//			log("TRACE", "  - ${index}: else: $l, format()=$fmt")
	...
	evts.eachWithIndex {l, index ->
		...
		else if (l in physicalgraph.device.HubAction) { trcStr = trcStr.concat("response: $l") }
